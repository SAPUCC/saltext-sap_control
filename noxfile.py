...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/sap_control/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/sap_control/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/sap_control/*"
            )
            ...
            #    "--include=src/saltext/sap_control/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/sap_control/config/schemas",
    )
